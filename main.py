...
class EnhancedAMRTracker:
    ...
    def __init__(
        ...
        self.pixel_size = self.config.measurement.pixel_size
...
def apply_preset_to_args(args: argparse.Namespace, preset: dict) -> argparse.Namespace:
    ...
    if args.source and "source" in preset:
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="I3ZEB19D",
        help="Device ID for camera (e.g., 'I3ZEB19D')",
    )
    parser.add_argument(
        "--source",
        default="0",
    ...
    # # Initialize system
    # if preset_config["loader_mode"] == "basic":
    ...
    #         pixel_size,
    #     )
    # if preset_config["mode"] == "enhanced":
    ...
    #         pixel_size = 1.0
    run_enhanced_mode(
            source=preset_config["source"],
            loader_mode=preset_config["loader_mode"],
...
#     # if preset_config["loader_mode"]  == "camera":
#     #     cap = create_camera_device_loader(source, config)
#     # elif preset_config["loader_mode"] == "video":
#     #     cap = create_video_file_loader(source)
#     # elif preset_config["loader_mode"] == "sequence":
#     #     cap = create_image_sequence_loader(source, fps)
#     # else:  # auto mode
#     cap = create_sequence_loader(source=source, fps=fps, loader_mode=loader_mode)
...
def run_enhanced_mode(
    ...
    if loader_mode == "camera":
        cap = create_camera_device_loader(source=source, config=config)
    elif loader_mode == "video":
        cap = create_video_file_loader(source=source)
    elif loader_mode == "sequence":
        cap = create_image_sequence_loader(source=source)
    else:  # auto mode
        cap = create_sequence_loader(source=source, loader_mode=loader_mode)
