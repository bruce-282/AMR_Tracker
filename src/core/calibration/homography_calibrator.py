...
class HomographyCalibrator:
    ...
    def calibrate_ground_plane(
        self, image: np.ndarray, pattern_size: Tuple[int, int], square_size: float
    ) -> Dict:
        """
        ...
        # Calculate pixel size (mm per pixel)
        self.pixel_size = self._calculate_pixel_size(img_points_2d, obj_points_2d)
    ...
    def _calculate_pixel_size(
        self, img_points: np.ndarray, obj_points: np.ndarray
    ) -> float:
        """Calculate pixel size (mm per pixel)."""
        ...
        # Calculate average pixel size (mm per pixel)
        pixel_sizes = [obj_d / img_d for img_d, obj_d in zip(img_dists, obj_dists)]
        return np.mean(pixel_sizes)
